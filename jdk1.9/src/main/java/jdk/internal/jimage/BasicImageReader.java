...
public class BasicImageReader implements AutoCloseable {
    ...
    protected BasicImageReader(Path path, ByteOrder byteOrder)
        ...
        if (USE_JVM_MAP && BasicImageReader.class.getClassLoader() == null) {
            ...
            map = jdk.internal.jimage.NativeImageBuffer.getNativeMap(name);
         } else {
            map = null;
        }
        ...
        int headerSize = jdk.internal.jimage.ImageHeader.getHeaderSize();
        ...
        stringsReader = new jdk.internal.jimage.ImageStringsReader(this);
    ...
    private jdk.internal.jimage.ImageHeader readHeader(IntBuffer buffer) throws IOException {
        jdk.internal.jimage.ImageHeader result = jdk.internal.jimage.ImageHeader.readFrom(buffer);

        if (result.getMagic() != jdk.internal.jimage.ImageHeader.MAGIC) {
            throw new IOException("\"" + name + "\" is not an image file");
        }

        if (result.getMajorVersion() != jdk.internal.jimage.ImageHeader.MAJOR_VERSION ||
            result.getMinorVersion() != jdk.internal.jimage.ImageHeader.MINOR_VERSION) {
    ...
    public static void releaseByteBuffer(ByteBuffer buffer) {
        ...
        if (!MAP_ALL) {
            jdk.internal.jimage.ImageBufferCache.releaseBuffer(buffer);
        }
    }
    ...
    public synchronized jdk.internal.jimage.ImageLocation findLocation(String module, String name) {
        ...
        int index = redirect.get(jdk.internal.jimage.ImageStringsReader.hashCode(module, name) % count);
        ...
        if (index < 0) {
            ...
            index = jdk.internal.jimage.ImageStringsReader.hashCode(module, name, index) % count;
        } else {
            // No entry.
            return null;
        }
        ...
        if (!jdk.internal.jimage.ImageLocation.verify(module, name, attributes, stringsReader)) {
            return null;
        }
        return new jdk.internal.jimage.ImageLocation(attributes, stringsReader);
    }

    public synchronized jdk.internal.jimage.ImageLocation findLocation(String name) {
        Objects.requireNonNull(name);
        // Details of the algorithm used here can be found in
        // jdk.tools.jlink.internal.PerfectHashBuilder.
        int count = header.getTableLength();
        int index = redirect.get(jdk.internal.jimage.ImageStringsReader.hashCode(name) % count);
        ...
        if (index < 0) {
            ...
            // index is hash seed needed to compute location attributes index.
            index = jdk.internal.jimage.ImageStringsReader.hashCode(name, index) % count;
        } else {
            // No entry.
            return null;
        }
        ...
        if (!jdk.internal.jimage.ImageLocation.verify(name, attributes, stringsReader)) {
            return null;
        }
        return new jdk.internal.jimage.ImageLocation(attributes, stringsReader);
    }
    ...
    public String[] getEntryNames() {
        ...
        return IntStream.of(attributeOffsets)
                        .filter(o -> o != 0)
                        .mapToObj(o -> jdk.internal.jimage.ImageLocation.readFrom(this, o).getFullName())
                        .sorted()
                        .toArray(String[]::new);
    }

    jdk.internal.jimage.ImageLocation getLocation(int offset) {
        return jdk.internal.jimage.ImageLocation.readFrom(this, offset);
    }
    ...
    public long[] getAttributes(int offset) {
        ...
        return jdk.internal.jimage.ImageLocation.decompress(buffer);
    }
    ...
    public String getString(int offset) {
        ...
        return jdk.internal.jimage.ImageStringsReader.stringFromByteBuffer(buffer);
    }
    ...
    private ByteBuffer readBuffer(long offset, long size) {
        ...
        if (MAP_ALL) {
            ...
            ByteBuffer buffer = jdk.internal.jimage.ImageBufferCache.getBuffer(size);
            int read;
            try {
    ...
    public byte[] getResource(String name) {
        Objects.requireNonNull(name);
        jdk.internal.jimage.ImageLocation location = findLocation(name);

        return location != null ? getResource(location) : null;
    }

    public byte[] getResource(jdk.internal.jimage.ImageLocation loc) {
    ...
    public ByteBuffer getResourceBuffer(jdk.internal.jimage.ImageLocation loc) {
    ...
    public InputStream getResourceStream(jdk.internal.jimage.ImageLocation loc) {
        Objects.requireNonNull(loc);
